...
public final class AdminCommand implements CommandExecutor, TabCompleter {
    ...
    public AdminCommand(CdrJobsPlugin plugin, Database database, ProfessionStore store, ProgressionService progression) {
        this.plugin = plugin;
    ...
    private boolean handleDiagnose(CommandSender sender, String[] args) {
        if (args.length != 1) {
            sender.sendMessage("§cUsage: /cdrjobsadmin diagnose");
            return true;
        }
    ...
    private boolean handleInspect(CommandSender sender, String[] args) {
        if (args.length != 2) {
            sender.sendMessage("§cUsage: /cdrjobsadmin inspect <player>");
            return true;
        }

        Player target = Bukkit.getPlayerExact(args[1]);
        if (target == null) {
            sender.sendMessage("§cPlayer harus online.");
            return true;
        }
        ...
        for (JobType job : JobType.values()) {
            ...
            String rank = JobRanks.title(job, progress.level());
            sender.sendMessage("§7- §b" + job.displayName() + " §8[§f" + job.name().toLowerCase(Locale.ROOT) + "§8] §7Lv.§f" + progress.level() + " §7XP §f" + progress.xp() + " §8• §d" + rank);

            if (progress.level() > highestLevel) {
                highestLevel = progress.level();
                highest.clear();
                highest.add(job.displayName());
            } else if (progress.level() == highestLevel) {
                highest.add(job.displayName());
            }
        }
    ...
    private boolean handleReload(CommandSender sender, String[] args) {
        if (args.length != 1) {
            sender.sendMessage("§cUsage: /cdrjobsadmin reload");
            return true;
        }
    ...
    private boolean handleReset(CommandSender sender, String[] args) {
        if (args.length != 2) {
            sender.sendMessage("§cUsage: /cdrjobsadmin reset <player>");
            return true;
        }
        Player target = Bukkit.getPlayerExact(args[1]);
        if (target == null) {
    ...
    private boolean handleProgressCommand(CommandSender sender, String[] args) {
        ...
        if (args.length == 4) {
            try {
                job = JobType.valueOf(args[2].toUpperCase(Locale.ROOT));
        ...
        try {
            if (addXp) {
                ...
                if (amount <= 0L) {
                    sender.sendMessage("§cXP harus lebih besar dari 0.");
                    return true;
                }
                ...
                if (level < 1 || level > maxLevel) {
                    sender.sendMessage("§cLevel harus berada di antara 1 dan " + maxLevel + ".");
                    return true;
                }
    ...
    public @Nullable List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        if (args.length == 1) {
            return filter(List.of("diagnose", "inspect", "reload", "addxp", "setlevel", "reset"), args[0]);
        }
        if (args.length == 2 && List.of("inspect", "addxp", "setlevel", "reset").contains(args[0].toLowerCase(Locale.ROOT))) {
            List<String> names = new ArrayList<>();
            Bukkit.getOnlinePlayers().forEach(player -> names.add(player.getName()));
            return filter(names, args[1]);
        }
        if (args.length == 3 && (args[0].equalsIgnoreCase("addxp") || args[0].equalsIgnoreCase("setlevel"))) {
            return filter(Arrays.stream(JobType.values()).map(job -> job.name().toLowerCase(Locale.ROOT)).toList(), args[2]);
        }
        return List.of();
    }
