...
public final class AdminCommand implements CommandExecutor, TabCompleter {
    ...
    private boolean handleInspect(CommandSender sender, String[] args) {
        ...
        for (JobType job : JobType.values()) {
            ...
            long rebirthCd = rebirth.cooldownRemainingSeconds(uuid, job);
            if (rebirthCd > 0) sender.sendMessage("  §7Rebirth cooldown: §f" + rebirthCd + "s");
    ...
    private boolean handleReset(CommandSender sender, String[] args) {
        ...
        sender.sendMessage("§aSeluruh data CdrJobs direset: " + target.getName());
        return true;
    }
    ...
    private boolean handleResetJob(CommandSender sender, String[] args) {
        ...
        database.resetProfession(target.getUniqueId(), job);
        store.resetProfessionData(target.getUniqueId(), job.name());
        sender.sendMessage("§aReset profession " + job.name() + " untuk " + target.getName() + ". Fate Essence global tidak dihapus.");
        return true;
    }
    ...
    private boolean handleResetTrial(CommandSender sender, String[] args) {
        ...
        JobType job = parseJob(sender, args[2]);
        if (job == null) return true;
        if (job == JobType.MINER) database.resetMinerTrial(target.getUniqueId());
        else store.resetTrialData(target.getUniqueId(), job.name());
        sender.sendMessage("§aTrial progress " + job.name() + " direset untuk " + target.getName() + ".");
        if (job != JobType.MINER) sender.sendMessage("§7Lifetime counter tetap disimpan; hanya baseline progress Trial yang direset.");
        return true;
    }
    ...
    private boolean handleResetCooldown(CommandSender sender, String[] args) {
        ...
        sender.sendMessage("§aAbility cooldown " + job.name() + " direset untuk " + target.getName() + ". Rebirth cooldown tidak ikut dihapus.");
        return true;
    }

    private boolean handleForceRebirth(CommandSender sender, String[] args) {
        if (args.length != 3) { sender.sendMessage("§cUsage: /cdrjobsadmin forcerespec <player> <job>"); return true; }
        Player target = Bukkit.getPlayerExact(args[1]);
        if (target == null) { sender.sendMessage("§cPlayer harus online."); return true; }
        JobType job = parseJob(sender, args[2]);
        if (job == null) return true;
        RebirthService.Outcome outcome = rebirth.force(target, job);
        if (outcome.status() == RebirthService.Status.SUCCESS) {
            sender.sendMessage("§aForce Rebirth " + job.name() + " untuk " + target.getName() + ". Refund: " + outcome.refundEssence() + " Fate Essence.");
    ...
    private boolean handleExport(CommandSender sender, String[] args) {
        ...
        try {
            if (mode.equals("file") || mode.equals("both")) {
                ...
                if (!dir.exists() && !dir.mkdirs()) throw new IllegalStateException("Could not create debug directory");
    ...
    private Player onlineTarget(CommandSender sender, String[] args, int exactLength, String usage) {
        ...
        Player target = Bukkit.getPlayerExact(args[1]);
        if (target == null) sender.sendMessage("§cPlayer harus online.");
        return target;
    }
