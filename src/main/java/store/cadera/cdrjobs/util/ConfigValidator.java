...
public final class ConfigValidator {
    ...
    public static void validate(CdrJobsPlugin plugin) {
        ...
        if (maxLevel < 1) {
            plugin.getLogger().warning("settings.max-level must be >= 1; runtime will clamp where possible.");
        }
        ...
        for (String job : JOBS) {
            ...
            if (multiplier < 0D) {
                plugin.getLogger().warning(job + ".xp-multiplier is negative; rewards will be clamped by profession logic.");
            }
        ...
        if (milestones != null) {
            ...
            for (String key : milestones.getKeys(false)) {
                try {
                    ...
                    if (level < 1 || level > safeMax) {
                        plugin.getLogger().warning("Fate Essence milestone " + level + " is outside the configured level range 1-" + safeMax + ".");
                    }
                    if (reward <= 0) {
                        plugin.getLogger().warning("Fate Essence milestone " + level + " should reward at least 1 Essence.");
                    }
        ...
        if (farmerCooldown < 0L) {
            plugin.getLogger().warning("Farmer location cooldown cannot be negative; runtime will clamp it.");
        }

        long fisherAfk = plugin.getConfig().getLong("fisher.anti-exploit.afk-after-seconds", 600L);
        if (fisherAfk < 60L) {
            plugin.getLogger().warning("Fisher AFK threshold below 60s is aggressive and may cause false positives.");
        }
    }
}
