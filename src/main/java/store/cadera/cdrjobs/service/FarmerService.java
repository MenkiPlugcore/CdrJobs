...
public final class FarmerService {
    ...
    public FarmerService(CdrJobsPlugin plugin, Database database, ProfessionStore store) {
        this.plugin = plugin; this.database = database; this.store = store;
    }

    public int rank(Player player, FarmerSkill skill) { return store.getSkillRank(player.getUniqueId(), skill.key()); }

    public boolean tryUpgrade(Player player, FarmerSkill skill) {
        int rank = rank(player, skill);
        if (rank >= skill.maxRank()) { player.sendMessage(Colors.color(plugin.prefix() + plugin.message("max-skill"))); return false; }
        int level = database.getProgress(player.getUniqueId(), JobType.FARMER).level();
        if (level < skill.requiredLevel()) { player.sendMessage(Colors.color(plugin.prefix() + plugin.message("skill-requirement"))); return false; }
        if (skill.prerequisite() != null && rank(player, skill.prerequisite()) < skill.prerequisiteRank()) {
            player.sendMessage(Colors.color(plugin.prefix() + plugin.message("skill-requirement"))); return false;
        }
        if (skill == FarmerSkill.VERDANT_BLOOM && !seedTrialComplete(player)) {
            player.sendMessage(Colors.color(plugin.prefix() + plugin.message("trial-required").replace("%trial%", "Trial of Seed"))); return false;
        }
        if (skill == FarmerSkill.VERDANT_DOMINION) {
            if (rank(player, FarmerSkill.BLESSING_OF_GAIA) < 3 || rank(player, FarmerSkill.SPIRIT_OF_GROVE) < 3 || !gaiaTrialComplete(player)) {
                player.sendMessage(Colors.color(plugin.prefix() + plugin.message("skill-requirement"))); return false;
            }
        }
        if (database.getFateEssence(player.getUniqueId()) < skill.essenceCost()) {
            player.sendMessage(Colors.color(plugin.prefix() + plugin.message("not-enough-essence"))); return false;
        }
        database.addFateEssence(player.getUniqueId(), -skill.essenceCost());
        store.setSkillRank(player.getUniqueId(), skill.key(), rank + 1);
        player.sendMessage(Colors.color(plugin.prefix() + plugin.message("skill-unlocked").replace("%skill%", skill.displayName()).replace("%rank%", String.valueOf(rank + 1))));
        return true;
    }
    ...
    public long applyXpModifiers(Player player, long baseXp) {
        ...
        if (until != null) {
            ...
            } else activeUntil.remove(player.getUniqueId());
        }
        return xp;
    }

    public void recordHarvest(Player player, boolean rare) {
        long harvests = store.incrementCounter(player.getUniqueId(), JOB, "harvests", 1);
        if (rare) store.incrementCounter(player.getUniqueId(), JOB, "rare_harvests", 1);
        if (!seedTrialComplete(player) && harvests >= seedTarget()) {
            store.setFlag(player.getUniqueId(), JOB, "trial_seed", true);
            player.sendMessage(Colors.color(plugin.prefix() + plugin.message("trial-seed-complete")));
        }
        if (!gaiaTrialComplete(player) && harvests >= gaiaHarvestTarget() && rareHarvests(player) >= gaiaRareTarget()) {
            store.setFlag(player.getUniqueId(), JOB, "trial_gaia", true);
    ...
    public boolean activate(Player player) {
        if (rank(player, FarmerSkill.VERDANT_BLOOM) <= 0) {
            player.sendMessage(Colors.color(plugin.prefix() + plugin.message("farmer-ability-locked"))); return false;
        }
        long now = System.currentTimeMillis();
        long ready = store.getAbilityReadyAt(player.getUniqueId(), VERDANT_BLOOM);
        if (ready > now) {
            long seconds = Math.max(1L, (ready - now + 999L) / 1000L);
            player.sendMessage(Colors.color(plugin.prefix() + plugin.message("farmer-ability-cooldown").replace("%seconds%", String.valueOf(seconds)))); return false;
        }
        ...
        player.sendMessage(Colors.color(plugin.prefix() + plugin.message("verdant-bloom-activated").replace("%duration%", String.valueOf(duration))));
        return true;
    }

    public boolean autoReplant(Player player) { return rank(player, FarmerSkill.ROOTBOUND) > 0; }
    public long harvests(Player p) { return store.getCounter(p.getUniqueId(), JOB, "harvests"); }
    public long rareHarvests(Player p) { return store.getCounter(p.getUniqueId(), JOB, "rare_harvests"); }
    public boolean seedTrialComplete(Player p) { return store.getFlag(p.getUniqueId(), JOB, "trial_seed"); }
    public boolean gaiaTrialComplete(Player p) { return store.getFlag(p.getUniqueId(), JOB, "trial_gaia"); }
    public int seedTarget() { return Math.max(1, plugin.getConfig().getInt("farmer.trials.trial-of-seed.harvests", 500)); }
    public int gaiaHarvestTarget() { return Math.max(1, plugin.getConfig().getInt("farmer.trials.trial-of-gaia.harvests", 2000)); }
    public int gaiaRareTarget() { return Math.max(1, plugin.getConfig().getInt("farmer.trials.trial-of-gaia.rare-harvests", 250)); }
