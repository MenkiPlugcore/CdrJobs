...
public final class HunterService {
    ...
    public boolean tryClaimPvpReward(Player killer, Player victim) {
        ...
        try {
            ...
            cooldownMillis = Long.MAX_VALUE - now;
        }
        long nextReady;
        try {
            nextReady = Math.addExact(now, cooldownMillis);
        } catch (ArithmeticException overflow) {
            nextReady = Long.MAX_VALUE;
        }
        store.setAbilityReadyAt(killer.getUniqueId(), key, nextReady);
        return true;
    }
    ...
    public boolean tryUpgrade(Player p, HunterSkill s) {
        ...
        if (s == HunterSkill.APEX_PREDATOR && (rank(p, HunterSkill.SOULMARK) < 3 || rank(p, HunterSkill.WARDENS_OATH) < 3 || !moonComplete(p))) {
            p.sendMessage(Colors.color(plugin.prefix() + plugin.message("skill-requirement")));
            return false;
        }
    ...
    public void recordKill(Player p, boolean dangerous, boolean night) {
        ...
        if (!fangComplete(p) && kills >= fangTarget()) {
            store.setFlag(p.getUniqueId(), JOB, "trial_fang", true);
            p.sendMessage(Colors.color(plugin.prefix() + plugin.message("trial-fang-complete")));
        }
        if (!moonComplete(p) && kills >= moonKillTarget() && nightKills(p) >= moonNightTarget() && dangerousKills(p) >= moonDangerTarget()) {
            store.setFlag(p.getUniqueId(), JOB, "trial_moon", true);
    ...
    public boolean activate(Player p) {
        ...
        if (ready > now) {
            p.sendMessage(Colors.color(plugin.prefix() + plugin.message("hunter-ability-cooldown").replace("%seconds%", String.valueOf((ready - now + 999) / 1000))));
            return false;
        }
    ...
    public long cooldownSeconds(UUID uuid) {
        long r = store.getAbilityReadyAt(uuid, CRIMSON_HUNT) - System.currentTimeMillis();
        return r <= 0 ? 0 : (r + 999) / 1000;
    }
}
