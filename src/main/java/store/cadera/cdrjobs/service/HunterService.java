...
public final class HunterService {
    ...
    public boolean tryUpgrade(Player p, HunterSkill s) {
        int r = rank(p, s);
        if (r >= s.maxRank()) { p.sendMessage(Colors.color(plugin.prefix() + plugin.message("max-skill"))); return false; }
        int lv = database.getProgress(p.getUniqueId(), JobType.HUNTER).level();
        if (lv < s.requiredLevel()) { p.sendMessage(Colors.color(plugin.prefix() + plugin.message("skill-requirement"))); return false; }
        if (s.prerequisite() != null && rank(p, s.prerequisite()) < s.prerequisiteRank()) { p.sendMessage(Colors.color(plugin.prefix() + plugin.message("skill-requirement"))); return false; }
        if (s == HunterSkill.CRIMSON_HUNT && !fangComplete(p)) { p.sendMessage(Colors.color(plugin.prefix() + plugin.message("trial-required").replace("%trial%", "Trial of Fang"))); return false; }
        if (s == HunterSkill.APEX_PREDATOR && (rank(p, HunterSkill.SOULMARK) < 3 || rank(p, HunterSkill.WARDENS_OATH) < 3 || !moonComplete(p))) { p.sendMessage(Colors.color(plugin.prefix() + plugin.message("skill-requirement"))); return false; }
        if (database.getFateEssence(p.getUniqueId()) < s.essenceCost()) { p.sendMessage(Colors.color(plugin.prefix() + plugin.message("not-enough-essence"))); return false; }
        database.addFateEssence(p.getUniqueId(), -s.essenceCost());
        store.setSkillRank(p.getUniqueId(), s.key(), r + 1);
        p.sendMessage(Colors.color(plugin.prefix() + plugin.message("skill-unlocked").replace("%skill%", s.displayName()).replace("%rank%", String.valueOf(r + 1))));
        return true;
    }
    ...
    public long applyXp(Player p, long base, boolean dangerous) {
        ...
        if (until != null) {
            ...
            } else activeUntil.remove(p.getUniqueId());
        }
        return xp;
    }

    public void recordKill(Player p, boolean dangerous, boolean night) {
        long kills = store.incrementCounter(p.getUniqueId(), JOB, "kills", 1);
        if (night) store.incrementCounter(p.getUniqueId(), JOB, "night_kills", 1);
        if (dangerous) store.incrementCounter(p.getUniqueId(), JOB, "dangerous_kills", 1);
        if (!fangComplete(p) && kills >= fangTarget()) { store.setFlag(p.getUniqueId(), JOB, "trial_fang", true); p.sendMessage(Colors.color(plugin.prefix() + plugin.message("trial-fang-complete"))); }
        if (!moonComplete(p) && kills >= moonKillTarget() && nightKills(p) >= moonNightTarget() && dangerousKills(p) >= moonDangerTarget()) { store.setFlag(p.getUniqueId(), JOB, "trial_moon", true); p.sendMessage(Colors.color(plugin.prefix() + plugin.message("trial-moon-complete"))); }
    }

    public boolean activate(Player p) {
        if (rank(p, HunterSkill.CRIMSON_HUNT) <= 0) { p.sendMessage(Colors.color(plugin.prefix() + plugin.message("hunter-ability-locked"))); return false; }
        long now = System.currentTimeMillis(), ready = store.getAbilityReadyAt(p.getUniqueId(), CRIMSON_HUNT);
        if (ready > now) { p.sendMessage(Colors.color(plugin.prefix() + plugin.message("hunter-ability-cooldown").replace("%seconds%", String.valueOf((ready - now + 999) / 1000)))); return false; }
        ...
        p.sendMessage(Colors.color(plugin.prefix() + plugin.message("crimson-hunt-activated").replace("%duration%", String.valueOf(duration))));
        return true;
    }

    public long kills(Player p) { return store.getCounter(p.getUniqueId(), JOB, "kills"); }
    public long nightKills(Player p) { return store.getCounter(p.getUniqueId(), JOB, "night_kills"); }
    public long dangerousKills(Player p) { return store.getCounter(p.getUniqueId(), JOB, "dangerous_kills"); }
    public boolean fangComplete(Player p) { return store.getFlag(p.getUniqueId(), JOB, "trial_fang"); }
    public boolean moonComplete(Player p) { return store.getFlag(p.getUniqueId(), JOB, "trial_moon"); }
    public int fangTarget() { return Math.max(1, plugin.getConfig().getInt("hunter.trials.trial-of-fang.kills", 300)); }
    public int moonKillTarget() { return Math.max(1, plugin.getConfig().getInt("hunter.trials.trial-of-crimson-moon.kills", 1200)); }
    public int moonNightTarget() { return Math.max(1, plugin.getConfig().getInt("hunter.trials.trial-of-crimson-moon.night-kills", 250)); }
    public int moonDangerTarget() { return Math.max(1, plugin.getConfig().getInt("hunter.trials.trial-of-crimson-moon.dangerous-kills", 10)); }
