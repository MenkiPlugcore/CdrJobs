...
public final class ProgressionService {
    ...
    private long saturatingAdd(long current, long amount) {
        if (amount > Long.MAX_VALUE - current) return Long.MAX_VALUE;
        return current + amount;
    }

    private void onLevelUp(Player player, JobType job, int oldLevel, int newLevel) {
        ...
        plugin.getServer().getPluginManager().callEvent(new ProfessionLevelUpEvent(player, job, oldLevel, newLevel));
    }

    public void setLevel(Player player, JobType job, int level) {
        int safe = Math.max(1, Math.min(levels.maxLevel(), level));
        database.setProgress(player.getUniqueId(), job, safe, 0L);
    }
}
